...
class GoldLightGBM:
    ...
    def fit(self, features_df: pd.DataFrame, prices: Optional[pd.Series] = None) -> Dict:
        ...
        try:
            ...
            if len(self.feature_cols) >= 5:
                logger.info(f"[LightGBM] 使用特征白名单 {len(self.feature_cols)} 个（降噪后）: {self.feature_cols}")
            else:
                self.feature_cols = all_cols
                logger.info(f"[LightGBM] 白名单未生效，回退全部 {len(all_cols)} 个特征")
        except Exception:
            self.feature_cols = all_cols
            logger.info(f"[LightGBM] 回退全部 {len(all_cols)} 个特征（config 未定义）")
        ...
        logger.info(f"[LightGBM] 数据量={n}，fold_size={fold_size}，CV folds={len(cv_splits)}")
        ...
        for h in self.horizons:
            if h >= n - fold_size:
                logger.warning(f"[LightGBM] horizon={h}d 数据不足，跳过")
                continue
            ...
            logger.info(f"[LightGBM] horizon={h}d | 上涨比例={pos_rate:.1%}")
            ...
            for fold_i, (train_idx, val_idx) in enumerate(cv_splits):
                ...
                model = lgb.LGBMClassifier(
                    ...
                    min_split_gain=self.gamma,  # xgboost gamma → lightgbm min_split_gain
            ...
            if fold_aucs:
                ...
                logger.info(
                    f"[LightGBM] horizon={h}d | CV AUC={mean_auc:.3f}±{std_auc:.3f} "
                ...
                logger.warning(f"[LightGBM] horizon={h}d CV 失败，跳过")
        ...
        logger.info(f"[LightGBM] 训练完成 {status}，耗时 {duration:.1f}s")
    ...
    def save(self, path: str):
        ...
        logger.info(f"[LightGBM] 模型已保存: {path}")

    @classmethod
    def load(cls, path: str) -> "GoldLightGBM":
        """从磁盘加载模型"""
        ...
        logger.info(f"[LightGBM] 模型已加载: {path}")
        return xgb_model
